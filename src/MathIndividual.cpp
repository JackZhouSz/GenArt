...
void MathIndividual::ImportColorMap(ColorMap<f3Pixel>* CMap_)
{
    ...
    for (size_t j = 0; j < CMap.size(); j++) {
        if (!isFinite(CMap[j])) CMap[j] = f3Pixel(0.0f, 0.0f, 0.0f);
    }
}
...
{
    switch (space) {
    case SPACE_RGB: cspaceInterval[0] = cspaceInterval[1] = cspaceInterval[2] = interval(0, 1); break;
    case SPACE_TONEMAP_RGB: cspaceInterval[0] = cspaceInterval[1] = cspaceInterval[2] = interval(0, interval::infinity()); break;
    case SPACE_YCRCB:
...
std::string MathIndividual::stringDisplay(const float x /*= -1000.0f*/, const float y /*= -1000.0f*/)
{
    ...
    if (!c || !inWindow) {
        ...
        for (size_t i = 0; i < CMap.size(); i++) { out << CMap[i] << std::endl; }
    }
    out << std::endl;
    return out.str();
}
...
MathIndividual::shp fromstream(std::ifstream& InF, bool onlyColorMaps, bool& finished)
{
    ...
    if (ColorSpace == SPACE_COLMAP || ColorSpace == SPACE_TONEMAP_COLMAP) {
        ...
        if (onlyColorMaps) { return NULL; }
    }

    MathIndividual::shp ind(new MathIndividual(Ch0, Ch1, Ch2, CMap, ColorSpace_t(ColorSpace), Score, IDNum, Generation, ParentA, ParentB, XMin, YMin, BoxWid));
